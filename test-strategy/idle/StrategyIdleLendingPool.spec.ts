...
async function deployGovTokenRegistry(deployedAddress: string | undefined): Promise<GovTokenRegistry> {
  ...
  if (deployedAddress) {
    ...
    const govTokenRegistryFactory = (await ethers.getContractFactory('GovTokenRegistry')) as GovTokenRegistry__factory;
    govTokenRegistry = await govTokenRegistryFactory
      .connect(deployerSigner)
      .deploy(process.env.COMP as string, process.env.IDLE as string, process.env.AAVE as string);
...
export async function testStrategyIdleLendingPool(
  ...
  await expect(strategy.aggregateOrders(p('10'), p('500'), p('9'), p('499'))).to.revertedWith(
  ...
  await expect(govTokenRegistry.registerGovToken('0xc84f7abe4904ee4f20a8c5dfa3cc4bf1829330ab')).to.emit(
    govTokenRegistry,
    'GovTokenRegistered'
  );
  const govTokenLength = (await govTokenRegistry.getGovTokens()).length;
  expect(govTokenLength).to.eq(4);

  console.log('\n>>> unregister governance token from GovTokenRegistry');
  await expect(govTokenRegistry.unregisterGovToken('0xc84f7abe4904ee4f20a8c5dfa3cc4bf1829330ab')).to.emit(
    govTokenRegistry,
    'GovTokenUnregistered'
  );
  const govTokenLength2 = (await govTokenRegistry.getGovTokens()).length;
