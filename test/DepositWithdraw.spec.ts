...
describe('DepositWithdraw', function () {
  async function fixture([admin]: Wallet[]) {
    const { rollupChain, pendingQueues, celr, weth } = await deployContracts(admin);
  ...
  it('should deposit and withdraw ERC20 and ETH', async function () {
    const { admin, rollupChain, pendingQueues, celr, weth } = await loadFixture(fixture);
    ...
    let [ehash, blockId, status] = await pendingQueues.pendingDeposits(0);
    ...
    [ehash, blockId, status] = await pendingQueues.pendingDeposits(1);
  ...
  it('should handle deposit transition queue correctly', async function () {
    ...
    [ehash, blockId, status] = await pendingQueues.pendingDeposits(2);
    h = solidityKeccak256(['address', 'uint32', 'uint256'], [users[1].address, 1, 300]);
    expect(ehash).to.equal(h);
    expect(blockId).to.equal(0);
    expect(status).to.equal(0);

    [ehash, blockId, status] = await pendingQueues.pendingDeposits(3);
    ...
    [, , status] = await pendingQueues.pendingDeposits(2);
    expect(status).to.equal(1);
    [, , status] = await pendingQueues.pendingDeposits(3);
    expect(status).to.equal(0);
