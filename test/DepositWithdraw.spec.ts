...
describe('DepositWithdraw', function () {
  async function fixture([admin]: Wallet[]) {
    const { rollupChain, priorityQueues, celr, weth } = await deployContracts(admin);
  ...
  it('should deposit and withdraw ERC20 and ETH', async function () {
    const { admin, rollupChain, priorityQueues, celr, weth } = await loadFixture(fixture);
    ...
    let [ehash, blockId, status] = await priorityQueues.pendingDeposits(0);
    ...
    [ehash, blockId, status] = await priorityQueues.pendingDeposits(1);
  ...
  it('should handle deposit transition queue correctly', async function () {
    ...
    [ehash, blockId, status] = await priorityQueues.pendingDeposits(2);
    h = solidityKeccak256(['address', 'uint32', 'uint256'], [users[1].address, 1, 300]);
    expect(ehash).to.equal(h);
    expect(blockId).to.equal(0);
    expect(status).to.equal(0);

    [ehash, blockId, status] = await priorityQueues.pendingDeposits(3);
    ...
    [, , status] = await priorityQueues.pendingDeposits(2);
    expect(status).to.equal(1);
    [, , status] = await priorityQueues.pendingDeposits(3);
    expect(status).to.equal(0);
