...
import "./dependencies/single-reward-token/PoolSingle.sol";
import "../uniswap-v2/interfaces/IUniswapV2Router02.sol";

contract StrategyBarnBridgeCreamPool is AbstractStrategy {
    using SafeERC20 for IERC20;
    using Address for address;

    // The address of junior token(e.g. bb_crDAI, bb_crUSDC)
    address public immutable jToken;

    // The latest stored junior token price
    uint256 public latestJTokenPrice;

    // The address of provider which sends supply token to cream leding pool
    ...
    PoolSingle immutable yieldFarmContract;
    ...
    constructor(
        ...
        yieldFarmContract = PoolSingle(_yieldFarm);
    }
    ...
    function sell(uint256 _sellAmount) internal override returns (uint256) {
        ...
        // Unstake junior token(e.g. bb_crUSDC, bb_crDAI) from Yield Farm
